...
func Friend(ctx context.Context, requestContext *app.RequestContext) {
	...
	client := user.NewFriendListClient(dial)
	response, err := client.FriendList(context.TODO(), &user.FriendListRequest{
		UserId: user_id,
		MyId:   strconv.Itoa(userClaim.ID),
	})
	if err != nil {
		log.Println(err)
		requestContext.JSON(consts.StatusOK, utils.H{
