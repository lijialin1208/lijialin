...
func Login(ctx context.Context, requestContext *app.RequestContext) {
	...
	defer dial.Close()
	client := user.NewLoginClient(dial)
	response, err := client.Login(context.TODO(), &user.LoginRequest{
		Username: username,
		Password: password,
	})
	if err != nil {
